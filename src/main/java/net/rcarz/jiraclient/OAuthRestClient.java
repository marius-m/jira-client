...
import net.rcarz.utils.CollectionUtils;
 ...
 */
public class OAuthRestClient implements RestClient {
    ...
    public OAuthRestClient(
        ...
        this.uri = uri;
        final JiraApi jiraApi = new JiraApi(uri.toString(), privateKey);
    ...
    private JSON request(OAuthRequest req) throws RestException, IOException {
        ...
        if (status >= 300)
            throw new RestException(
                    ...
                    status,
                    result,
                    CollectionUtils.toHeaderArray(response.getHeaders())
            );
    ...
    private JSON request(OAuthRequest req, Issue.NewAttachment... attachments)
            throws RestException, IOException {
